...
// Salt + pepper SHA-256, identical scheme to the auth service so hashes are
// cross-verifiable. Stored format: `<base64-salt>$<hex-hash>` where
// hash = SHA-256(salt || pepper || plaintext). The pepper is shared with auth via the
// PASSWORD_PEPPER environment variable. This service only verifies (delete, password
// change) and re-hashes on password change; it never issues new local accounts.
@Service
